...
class DashBoardProvider extends ChangeNotifier {
  ...
  //filter the data depending on the parent dropdown value(parent->category, varianttype) 
  ...
  void addProduct() async{
    try{
      if(selectedMainImage==null){  //main image selection is mandatory
      ...
      final response = await service.addItem(endpointUrl: 'products', itemData: form);  //send a post request to the server
      if(response.isOk){
  ...
  void updateProduct() async{
    try{
      ...
      if(productForUpdate != null){}
      final response = await service.updateItem(endpointUrl: 'products', 
                        itemData: form, itemId: '${productForUpdate?.sId}');  //send a post request to the server
      if(response.isOk){
        ApiResponse apiResponse = ApiResponse.fromJson(response.body, null);  //The response body is parsed into an ApiResponse object 
        if(apiResponse.success==true){
          clearFields();
  ...
  void deleteProduct(Product product) async{
    try{
      Response response = await service.deleteItem(endpointUrl: 'products', 
      itemId: product.sId ?? '');
      if(response.isOk){
  ...
  void filterSubcategory(Category category){
    ...
    final newList = _dataProvider.subCategories.where(
      (subcategory) => subcategory.categoryId?.sId 
      == category.sId).toList();
  ...
  void filterVariant(VariantType variantType){
    ...
    final newList = _dataProvider.variants.where(
      (variant) => variant.variantTypeId?.sId 
      == variantType.sId).toList();
