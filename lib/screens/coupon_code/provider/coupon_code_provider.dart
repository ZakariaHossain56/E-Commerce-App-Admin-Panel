  ...
  void addCoupon() async{
    try{
      if(endDateCtrl.text.isEmpty){
        SnackBarHelper.showErrorSnackBar("Please select an end date!");  //like a toast
        return;
      }
      ...
      final response = await service.addItem(endpointUrl: 'couponCodes', itemData: coupon);  //send a post request to the server
      if(response.isOk){
  ...
  void updateCoupon() async{
    try{
      ...
      final response = await service.updateItem(endpointUrl: 'couponCodes', 
        itemId: couponForUpdate?.sId ?? '', itemData: coupon);  //send a post request to the server
      if(response.isOk){
        ApiResponse apiResponse = ApiResponse.fromJson(response.body, null);  //The response body is parsed into an ApiResponse object 
        if(apiResponse.success==true){
          clearFields();
