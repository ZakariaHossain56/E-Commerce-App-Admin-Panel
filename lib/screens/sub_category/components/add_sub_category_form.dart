                    ...
                    child: Consumer<SubCategoryProvider>(
                      builder: (context, subCatProvider, child) {
                        ...
                        return CustomDropdown(
                          ...
                          items: context.dataProvider.categories, //list of categories
                          displayItem: (Category? category) => category?.name ?? '',  //display the names of the categories in the dropdown
                          onChanged: (newValue) { //triggered when the user selects an item
                            if (newValue != null) {
                              subCatProvider.selectedCategory = newValue;
                              subCatProvider.updateUi();  //refresh the UI and reflect the change
                            }
                          },
                          validator: (value) {
                            if (value == null) {   //if no item is selected
                              return 'Please select a category';
                            }
                            return null;  //if an item is selected -> no validation error
                          },
                        );
                      },
                    ),
